...
const AddStaffModal = ({ isOpen, onClose }: AddStaffModalProps) => {
  ...
  const canSubmit = name.trim() !== '' && email.trim() !== '' && selectedRole !== null;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        ...
        {/* Title */}
        <h2 className="text-xl font-semibold text-gray-900">Add a staff member</h2>
        <p className="text-sm text-gray-500 mt-1">Invite them by email, or create the account directly.</p>
        ...
        <div className="mt-5">
          ...
          <div className="grid grid-cols-2 gap-3 mt-2">
            {ROLE_OPTIONS.map(r => (
              ...
              >
                <span className={`text-sm font-semibold ${selectedRole === r.value ? 'text-[#93406B]' : 'text-gray-900'}`}>
                  {r.value}
                </span>
